...
const ClubManagerApproval = () => {
    ...
    const {data:clubManager,refetch} = useQuery({
    ...
    const updateClubManagerStatus = (manager,status)=>{
        ...
        axiosSecure.patch(`/clubManager/${manager._id}`,updateInfo)
        .then(res=>{
    ...
    return (
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-10 text-center">
          <h1 className="text-4xl font-bold text-gray-900 ">Club Manager Applications</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-10">
          <div className="bg-white rounded-2xl p-6 shadow-lg text-center">
            <div className="text-3xl font-bold text-main">{pendingManager?.length}</div>
            <p className="text-gray-600 mt-1">Pending Approval</p>
          </div>
          <div className="bg-white rounded-2xl p-6 shadow-lg text-center">
            <div className="text-3xl font-bold text-green-600">{approvedManager?.length}</div>
            <p className="text-gray-600 mt-1">Approved Managers</p>
          </div>
          <div className="bg-white rounded-2xl p-6 shadow-lg text-center">
            <div className="text-3xl font-bold text-red-600">{rejectedManager?.length}</div>
        ...
        <div className="bg-white rounded-3xl shadow-xl overflow-hidden">
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-6 py-4 text-center text-sm font-semibold text-gray-700">Applied</th>
                  ...
                  <tr key={app._id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-6 py-5">
                      <div className="flex items-center justify-center gap-3">
                        <button onClick={()=>handleApprove(app)} className="p-3 bg-green-100 text-green-700 rounded-xl hover:bg-green-200 transition group">
                          <Check className="w-5 h-5 group-hover:scale-110 transition" />
                        </button>
                        <button onClick={()=>handleReject(app)} className="p-3 bg-red-100 text-red-700 rounded-xl hover:bg-red-200 transition group">
          ...
          {clubManager?.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="text-gray-500 text-lg">No pending applications at the moment.</p>
            </div>
          )}
        </div>
      </div>
